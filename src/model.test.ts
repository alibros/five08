...
describe('project import',()=>{
  it('sanitizes untrusted data without mutating it',()=>{const raw:any={version:2,name:'Unsafe',panel:{hp:999,widthMode:'bad',customWidth:-2,thickness:99,material:'Aluminium',finish:'brushed-silver',mounting:'bad'},panelColor:'red',inkColor:'#112233',accentColor:'#445566',items:[{id:'same',componentId:'knob-medium',x:'12',y:20,width:-5,height:9999,role:'admin',label:'x'.repeat(300)},{id:'same',componentId:'knob-medium'}],notes:'ok'};const p=parseProject(raw,catalogMap);expect(p.panel.hp).toBe(84);expect(p.panel.widthMode).toBe('doepfer');expect(p.panel.mounting).toBe('four');expect(p.panelColor).toBe('#d9d8d1');expect(p.items[0].width).toBe(.5);expect(p.items[0].role).toBe('none');expect(p.items[0].label.length).toBe(200);expect(p.items[0].id).not.toBe(p.items[1].id);expect(raw.panel.hp).toBe(999);});
